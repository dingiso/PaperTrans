...
对   其他调研的工具， 少代码将需要某种手动干预来克服， 许手动提供与 HALucinator 相同的功能
...
在调研的作品中，angr Ghidra 和 HALucinator 的框架中内置   库   配 其他作品要么不需要克服这   问题，要么没有   决功能识别问题	angr Ghidra 和 HALucinator 将使用   有技术来尝试识别某   功能，例如使用 IDA
...
调研的工具，angr 将转换为 VEX 这   技术中的每一种都有⾃⼰的挑战和权衡，这就是为什么在这一领域仍有研
究的原因
对   每	调研的工具，都有一   基础模拟器	angr Avatar2  CostinFA Firmadyne
...
QEMU，并使用硬件对 QEMU 进行   扩展，以允许将外设访问转发到   际硬件	Simics 通常是
...
内存或外设访问   际设备 如果您需要并行   您的仿真，HITL可能不可行 在这种情况下，从业   可能不得不	他们⾃⼰的时间   排	的时序可以是周期 指令 基础块或类似的东西 在许多情况下，时序与启用中
...
分析和后期仿真包括在⼆进制强	漏洞检测和其他网络   全工作⽅面的大量工作 所有   调研的工具都进行
某种后期仿真分析
...
和控制来缩小工具使用范围时留下   明确的路径，因为我们发   各种工具的意见差异很大.图左边一   大子里是所有   调研的工具 从那里   随箭头给出   模拟器之间的关键   别 如果分析师对机器学   感
...
是回答手头仿真问题的可行选择 在这种情况下，angr 是接受调研的最佳选择，因为   有一   专门的 Slack[154]小组，提供帮助和开发渠道，对用户很友好 如果符号执行不起作用，从业   可以参   图 1 以帮助缩小可用   所需精度的调研工具
...
使用模糊或随机   的工具控制较少，在某   情况下可能对从业   有用， 可能没用 在基础上所有调研的工具中，调试都内置在基础模拟器中和/或通过插件或默认内置提供
...
除   比较技术，我们还展示  调研作品的分类，包括angr Avatar2  CostinFA Firmadyne Ghidra HALucinatorMeunch2018P2 IM PANDA讨论Pretender这   技术PROSPECT及其差异，因为我QEMU们介绍Simics在仿真SURROGATES和固件重新托管和期间面临的核心挑战，为新的从业   或研究人员提供   一   概述，以	从哪里开始以及在固件重新托管和仿
...
嵌入式系统，但我们注意到   调研工具所做的英勇努力，并认识到使工具更   易和⾃动   以供从业
使用的重要性
...
这个作者太喜欢用 enable 了建议叫他 enabler 中  
